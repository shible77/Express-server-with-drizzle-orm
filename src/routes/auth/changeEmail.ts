...
changeEmailRouter.post('/changeEmail', async(req, res) => {
    try{
        ...
        await db.update(verify_user).set({email}).where(eq(verify_user.id, verification_id))
        const data = await db.select({verification_code : verify_user.verification_code}).from(verify_user).where(eq(verify_user.id,verification_id)).limit(1)
       sendVerificationEmail(email, data[0].verification_code)
       .then(() => {
