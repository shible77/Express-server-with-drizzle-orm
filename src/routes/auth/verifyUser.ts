...
verifyUserRouter.post("/verify", async(req, res) => {
    try{
        const { verification_id, verification_code } = verifyReqBody.parse(req.body)
        const user = await db
        .select({username : verify_user.username, email : verify_user.email, password : verify_user.password, code:verify_user.verification_code})
        .from(verify_user)
        .where(eq(verify_user.id, verification_id))
        .limit(1)

        if(user.length === 0 || user[0].code !== verification_code){
